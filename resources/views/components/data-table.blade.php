...
Simpan sebagai: resources/views/components/data-table.blade.php

Cara pakai di view mana saja:
  <x-data-table :columns="$columns" :rows="$rows" title="Pengguna" :per-page="10" />
...
Pastikan layout utama punya dua stacks:
  <head>
    ...
    @stack('styles')      ← taruh di dalam <head>
  </head>
  <body>
    ...
    @stack('scripts')     ← taruh sebelum </body>
...
{{-- ============================================================
     STYLES — di-push ke <head> layout via @stack('styles')
     ============================================================ --}}
@push('styles')
<style>
...
<div
    ...
    x-data="dataTable({{ json_encode($columns) }}, {{ json_encode($rows) }}, {{ (int) $perPage }})"
    x-init="init()"
    x-cloak
>
        ...
        <div class="flex items-center gap-2 shrink-0">
            ...
            <div class="relative" x-data="{ open: false }">
                <button
                    ...
                    class="flex items-center gap-1.5 px-3 py-2 text-sm border border-zinc-200
                           rounded-lg bg-white hover:bg-zinc-50 transition select-none"
                >
                    <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none"
                         viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round"
                              d="M12 3c2.755 0 5.455.232 8.083.678.533.09.917.556.917 1.096v1.044a2.25
                                 2.25 0 0 1-.659 1.591L15.75 12.4A2.25 2.25 0 0 0 15 14v6.25a.75.75 0 0
                                 1-1.079.672l-3-1.5A.75.75 0 0 1 10.5 18.75V14a2.25 2.25 0 0
                                 0-.659-1.591L4.659 7.409A2.25 2.25 0 0 1 4 5.818V4.774c0-.54.384-1.006.917-1.096A48.32
                                 48.32 0 0 1 12 3Z"/>
                    </svg>
                    <span x-text="statusFilter ? statusFilter : 'Filter'"></span>
                    <svg class="w-3.5 h-3.5 text-zinc-400" xmlns="http://www.w3.org/2000/svg"
                ...
                <div
                    ...
                    class="absolute right-0 mt-1 w-36 bg-white border border-zinc-200 rounded-lg
                           shadow-md z-10 py-1 text-sm"
                >
                    <button @click="statusFilter=''; open=false; currentPage=1"
                            class="w-full text-left px-3 py-1.5 hover:bg-zinc-50 text-zinc-500">Semua</button>
            ...
            <div class="relative" x-data="{ open: false }">
                <button
                    @click="open = !open"
                    class="flex items-center gap-1.5 px-3 py-2 text-sm border border-zinc-200
                           rounded-lg bg-white hover:bg-zinc-50 transition text-zinc-600 select-none"
                >
                    <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none"
                         viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <div
                    ...
                    x-transition:enter-start="opacity-0 scale-95"
                    x-transition:enter-end="opacity-100 scale-100"
                    class="absolute right-0 mt-1 w-44 bg-white border border-zinc-200 rounded-lg
                           shadow-md z-10 py-2 px-1 text-sm"
                >
                    <template x-for="col in columns" :key="col.key">
                        <label class="flex items-center gap-2 px-2 py-1.5 rounded-md
                                      hover:bg-zinc-50 cursor-pointer select-none">
                        ...
                        >
                            <div class="flex items-center gap-1.5">
                                ...
                                <template x-if="col.sortable">
                                    <svg
                                        class="w-3.5 h-3.5 dt-sort-icon"
                                    ...
                                    >
                                        <path fill-rule="evenodd"
                                              d="M11.354 4.646a.5.5 0 0 1 0 .708l-3 3a.5.5 0 0 1-.708
            ...
            <tbody>
                ...
                <template x-for="(row, i) in paginatedRows()" :key="rowKey(row, i)">
                    ...
                    >
                        <td class="px-4 py-3">
                            <input type="checkbox" class="dt-checkbox"
                                   :value="rowKey(row, i)" x-model="selected"/>
                        </td>
                        ...
                        <td class="px-3 py-3">
                            <div class="relative flex justify-end" x-data="{ open: false }">
                                <button
                                    ...
                                    class="p-1.5 rounded-md text-zinc-400 hover:text-zinc-700
                                           hover:bg-zinc-100 transition"
                                >
                                    <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg"
                                         viewBox="0 0 20 20" fill="currentColor">
                                        <path d="M10 3a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM10
                                                 8.5a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM11.5
                                                 15.5a1.5 1.5 0 1 0-3 0 1.5 1.5 0 0 0 3 0Z"/>
                                    </svg>
                                </button>
                                <div
                                    x-show="open" @click.outside="open = false"
                                    ...
                                    x-transition:enter-start="opacity-0 scale-95"
                                    x-transition:enter-end="opacity-100 scale-100"
                                    class="absolute right-0 top-8 w-36 bg-white border border-zinc-200
                                           rounded-lg shadow-lg z-20 py-1 text-sm text-zinc-700"
                                >
                                        ...
                                        <svg class="w-3.5 h-3.5 text-zinc-400" xmlns="http://www.w3.org/2000/svg"
                                            ...
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                  d="M15.75 17.25v3.375c0 .621-.504 1.125-1.125 1.125h-9.75a1.125
                                                     ...
                                                     1-1.125-1.125v-9.25m12 6.625v-1.875a3.375 3.375 0 0
                                                     0-3.375-3.375h-1.5a1.125 1.125 0 0 1-1.125-1.125v-1.5a3.375
                                    ...
                                    <button @click="deleteRow(row); open=false"
                                            class="w-full flex items-center gap-2 px-3 py-1.5
                                                   hover:bg-red-50 text-left text-red-600">
                                        <svg class="w-3.5 h-3.5" xmlns="http://www.w3.org/2000/svg"
                                            ...
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                  d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107
                                                     ...
                                                     0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0
                                                     0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5
    ...
    <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mt-4 text-sm text-zinc-500">
        ...
        <div class="flex items-center gap-1">
            <button @click="currentPage = 1" :disabled="currentPage === 1"
                    class="p-1.5 rounded-md border border-zinc-200 bg-white hover:bg-zinc-50
                           disabled:opacity-30 disabled:cursor-not-allowed transition" title="Pertama">
                <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd"
                          d="M4.72 9.47a.75.75 0 0 0 0 1.06l4.25 4.25a.75.75 0 1 0 1.06-1.06L6.31 10l3.72-3.72a.75.75
                             0 1 0-1.06-1.06L4.72 9.47Zm9.25-4.78a.75.75 0 0 1 0 1.06L10.25 9.47a.75.75 0 0 0 0
                             1.06l3.72 3.72a.75.75 0 1 1-1.06 1.06l-4.25-4.25a.75.75 0 0 1 0-1.06l4.25-4.25a.75.75
            ...
            <button @click="prevPage()" :disabled="currentPage === 1"
                ...
                <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd"
                          d="M11.78 5.22a.75.75 0 0 1 0 1.06L8.06 10l3.72 3.72a.75.75 0 1 1-1.06 1.06l-4.25-4.25a.75.75
                             0 0 1 0-1.06l4.25-4.25a.75.75 0 0 1 1.06 0Z" clip-rule="evenodd"/>
                </svg>
            </button>
            ...
            <button @click="nextPage()" :disabled="currentPage === totalPages()"
                ...
                <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd"
                          d="M8.22 5.22a.75.75 0 0 1 1.06 0l4.25 4.25a.75.75 0 0 1 0 1.06l-4.25 4.25a.75.75 0 0
                             1-1.06-1.06L11.94 10 8.22 6.28a.75.75 0 0 1 0-1.06Z" clip-rule="evenodd"/>
                </svg>
            </button>
            <button @click="currentPage = totalPages()" :disabled="currentPage === totalPages()"
                    class="p-1.5 rounded-md border border-zinc-200 bg-white hover:bg-zinc-50
                           disabled:opacity-30 disabled:cursor-not-allowed transition" title="Terakhir">
                <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd"
                          d="M15.28 9.47a.75.75 0 0 1 0 1.06l-4.25 4.25a.75.75 0 1 1-1.06-1.06L13.69 10 9.97
                             6.28a.75.75 0 0 1 1.06-1.06l4.25 4.25ZM6.03 4.69a.75.75 0 0 0 0 1.06l3.72
                             3.72a.75.75 0 0 1 0 1.06l-3.72 3.72a.75.75 0 1 0 1.06 1.06l4.25-4.25a.75.75 0 0 0
                             0-1.06L7.09 4.69a.75.75 0 0 0-1.06 0Z" clip-rule="evenodd"/>
                </svg>
            </button>
        </div>
    </div>
