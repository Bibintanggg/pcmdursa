        ...
        <div x-show="selected === {{ $org->id }}" x-transition:enter="transition-all duration-200"
            ...
            <div class="flex justify-between items-center px-6 py-5 border-b border-gray-100">
                ...
                <button @click="selected = null"
                    class="text-[11px] uppercase tracking-[.06em] px-4 py-2 border border-gray-200 rounded-full text-gray-500 hover:bg-gray-50 transition-colors">
                    Tutup
                </button>
            </div>
            ...
            <div class="grid grid-cols-3 divide-x divide-gray-100">
                @foreach ([['Ketua', $org->ketua], ['Sekretaris', $org->sekretaris], ['Bendahara', $org->bendahara]] as [$jabatan, $nama])
                    <div class="px-6 py-5">
                        <div class="text-[10px] uppercase tracking-[.08em] text-gray-400 mb-3">{{ $jabatan }}
                        </div>
                        <div class="text-sm font-medium">{{ $nama ?? '—' }}</div>
                        <div class="text-[11px] text-gray-400 mt-0.5">Periode
                ...
                <div class="px-6 py-5 border-t border-gray-100">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @foreach ($org->sub_units as $sub)
                            <span
                                class="text-[11px] px-3 py-1 border border-gray-200 rounded-full text-gray-600">{{ $sub->nama }}</span>
                        @endforeach
                    </div>
                </div>
            @endif
